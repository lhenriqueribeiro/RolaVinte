...
| Versão | Data | Resumo |
|---|---|---|
| [v0.9.0](v0.9.0.md) | 2026-08-10 | Sprint 4: o mestre conduz uma luta pela plataforma — agregado `Combate` (ordem de iniciativa, rodada, turno) com painel visível a todos, iniciativa como **resposta do sistema** (Percepção no PF2e, Destreza no D&D 5e, fechando o `rolagensPadrao` órfão de quatro versões), 14 condições de token com rótulo textual, e dano/cura por participante marcando `inconsciente` a 0 PV. As migrations `0011` e `0012` foram aplicadas **dentro** da sprint. O RV-140 alinhou as nove regras de `.claude/rules/` ao código — elas foram escritas antes da primeira linha de produção e nunca revisadas. Os dois primeiros itens planejados (RV-160, o card protetor, e RV-161) **não rodaram** (1799 testes verdes). |
