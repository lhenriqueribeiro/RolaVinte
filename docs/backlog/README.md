...
- Pathfinder 2e (sistema): **RV-151** — motor de regras puro em `packages/shared/src/sistemas/pathfinder2e/regras.ts`: `bonusProficiencia` (destreinado = 0, **sem somar o nível**, modelado como coluna da tabela e não como `if`), `grauSucesso` (compara com a CD primeiro e desloca **um grau** no 20/1 natural depois, sem transbordar nas pontas), `CDS_SIMPLES`, `cdPorNivel` (tabela 0..25 literal), `somarModificadores` (maior bônus e pior penalidade por tipo) e `d20NaturalDe` (devolve `null` sempre que houver ambiguidade). **RV-152** — `'pathfinder2e'` em `SISTEMAS_RPG` e no registro, ficha com identidade em texto livre, seis modificadores de atributo em −5..+8 como chaves de topo de `dados`, e `usaAtributosComuns: false` (o PF2e ignora as colunas comuns 1..30, então a ficha não oferece o botão genérico de teste de atributo). **RV-153** — 16 perícias de chave fixa mais o **Saber como família** (`familiasPericia`, instâncias em `dados.saberes`, "Saber (Guerra)" perito e "Saber (Náutico)" destreinado convivendo com bônus diferentes), bônus calculado pelo motor do RV-151 e rolagem em um clique pela rota que já existia; ações que exigem treinamento aparecem **indisponíveis com o motivo escrito**. Percepção não é perícia no PF2e (é RV-155). Nenhuma rota, DTO ou evento WS novo.
...
- Fundação: **RV-001** (ESLint com fronteiras de camada + Prettier), **RV-002** (CI), **RV-003** (harness de contrato HTTP com fakes em memória), **RV-004** (helmet, rate limit e body limit), **RV-005** (erro global `{ erro, requisicaoId }`, request id do servidor e logs redigidos) e **RV-008** (Vitest + Testing Library no `apps/web`) concluídos — `npm run check` roda lint + typecheck e `npm run test` cobre os **três** workspaces, com 1167 testes verdes (372 shared / 496 api / 299 web).
...
- As limitações abertas estão em [docs/release-notes/v0.7.0.md](../release-notes/v0.7.0.md).
...
| [E09](09-fichas.md) | RV-090, RV-091, RV-093, RV-096 |
| [E11](11-tempo-real.md) | RV-112, RV-115, RV-116 |
| [E12](12-ux.md) | RV-122 |
| [E13](13-operacao.md) | RV-138 |
| [E15](15-pathfinder2e.md) | RV-150, RV-151, RV-152, RV-153 |
...
| [E09](09-fichas.md) | Fichas e sistemas de RPG | RV-090 … RV-097 | 4 de 8 |
...
| [E15](15-pathfinder2e.md) | Pathfinder 2e | RV-150 … RV-159 | 4 de 10 |
...
**O próximo é o RV-154, e ele tem urgência de projeto:** `grauSucesso`, `d20NaturalDe`, `somarModificadores` e `cdPorNivel` existem, estão corretos (conferidos por execução na verificação da v0.7.0) e **não são chamados por nenhuma linha de produção**. Enquanto isso durar, o 20 natural não desloca grau nenhum na tela; e quanto mais tarde o RV-154 chegar, maior a chance de alguém reimplementar a comparação com a CD dentro do componente e criar uma segunda aritmética.
